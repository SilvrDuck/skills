---
name: defossil
description: Strip conversation-shaped fossils — text that's in an artifact only because of how it was iteratively produced, not because the artifact needs it (changelog comments, "now uses X instead of Y", defensive reassurances, "as we discussed" TODOs). Use when the user types `/defossil`, asks to scrub / launder / depersonalize an artifact, or says "make this look like it wasn't iterated on". ALSO auto-invoke before writing or editing a README, docstring, code comment, commit message, or PR description, before reviewing your own recent work, and before claiming a task complete — even if the user didn't ask. Be pushy: when in doubt whether something you're about to write is fossil-shaped, invoke this skill instead of guessing.
---
